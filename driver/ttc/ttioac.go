...
type tTIoac struct {
	...
	codepointLengthLimit common.UB4
...
func newTTIOAcDefine(
	...
	flags common.UB8,
	prefetchSize common.UB4,
) *tTIoac {
	oac := newTTIoac(typ, maxLength)
	oac.characterSetForm = common.UB1(colCtx.CharsetForm)
	oac.characterSetID = common.UB2(colCtx.CharsetID)
...
func newTTIOacString(maxLength common.UB4) common.Marshallable {
	if maxLength == 0 {
...
func newTTIOacBlobDefine(columnContext ColumnContext, lobPrefetchSize common.UB4) common.Marshallable {
	return newTTIOAcDefine(columnContext.DataType, max_lob_length, columnContext, 0, lobPrefetchSize)
}

// newTTIOacVarcharDefine creates a define OAC descriptor for VARCHAR-like scalar columns.
func newTTIOacVarcharDefine(columnContext ColumnContext) common.Marshallable {
...
func newTTIOacScalarDefine(columnContext ColumnContext) common.Marshallable {
...
func (p *tTIoac) MarshalTo(ctx context.Context, mar common.Marshaller) error {
	...
	if p.dataType == common.UB1(DtyNum) ||
		...
		if err := mar.MarshalUB1(ctx, common.UB1(p.scale)); err != nil {
	...
	if p.toid != nil {
		...
		if err := dalc.MarshalTo(ctx, mar); err != nil {
			...
			return common.NewOracleError(common.FailMarshal, err, "TTIOAC")
		}
	} else {
...
func (p *tTIoac) UnMarshalFrom(ctx context.Context, mar common.Marshaller) error {
	...
	if p.dataType, err = mar.UnmarshalUB1(ctx); err != nil {
		...
		return common.NewOracleError(common.FailUnmarshal, err, "OAC")
	}
...
func (p *tTIoac) addFlagsContinuation(flag common.UB8) {
